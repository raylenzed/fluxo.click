#!/usr/bin/env bash
# ============================================================
# Vortex - Direct Install Script
# ============================================================
# Installs Mihomo (proxy core) + Vortex (web dashboard)
...
log_banner()  {
  ...
  echo -e "${PURPLE}${BOLD}║              Vortex — Install Script                 ║${NC}"
  echo -e "${PURPLE}${BOLD}╚══════════════════════════════════════════════════════╝${NC}"
  echo -e ""
}
...
# Mihomo configuration — generated by Vortex installer
# Edit via the Vortex web UI or directly here.
...
install_vortex() {
  log_step "Installing Vortex web dashboard"

  mkdir -p "$DATA_DIR"

  # Check if already installed
  if [[ -d "$INSTALL_DIR" ]]; then
    log_warn "Vortex already installed at $INSTALL_DIR"
    if ! confirm "Re-install / upgrade?"; then
      log_info "Skipping Vortex install"
      return 0
    fi
  ...
  log_info "Vortex built successfully"
}
...
[Unit]
Description=Vortex - Web Dashboard for Mihomo
...
Environment=DB_PATH=${DATA_DIR}/vortex.db
...
WantedBy=multi-user.target
EOF
  log_info "Created /etc/systemd/system/vortex.service"

  # Reload and enable
  systemctl daemon-reload
  systemctl enable mihomo.service
  systemctl enable vortex.service
  ...
  log_detail "Starting vortex..."
  systemctl start vortex.service
  sleep 2

  if systemctl is-active --quiet vortex.service; then
    log_info "Vortex is running"
  else
    log_warn "Vortex failed to start — check: journalctl -u vortex -n 50"
  fi
}
...
configure_firewall() {
  ...
  if command -v ufw &>/dev/null && ufw status | grep -q "active"; then
    ...
    if confirm "Open ports ${WEB_PORT} (UI), ${SERVER_PORT} (API), 7890 (proxy) in ufw?"; then
      ufw allow "$WEB_PORT/tcp"   comment "vortex UI"   &>/dev/null
      ufw allow "$SERVER_PORT/tcp" comment "vortex API"  &>/dev/null
...
show_summary() {
  ...
  echo -e "  ${CYAN}systemctl status vortex${NC}           — dashboard status"
  echo -e "  ${CYAN}journalctl -fu mihomo${NC}             — core logs (live)"
  echo -e "  ${CYAN}journalctl -fu vortex${NC}             — dashboard logs (live)"
...
uninstall() {
  log_banner
  log_step "Uninstalling Vortex"

  echo -e "${RED}${BOLD}  WARNING: This will remove:${NC}"
  echo -e "  • Mihomo binary ($MIHOMO_BINARY)"
  echo -e "  • Vortex ($INSTALL_DIR)"
  echo -e "  • Vortex data ($DATA_DIR)"
