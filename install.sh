#!/usr/bin/env bash
# ============================================================
# Mihomo Party - Direct Install Script
# ============================================================
# Installs Mihomo (proxy core) + Mihomo Party (web dashboard)
...
NODE_VERSION="22"
INSTALL_DIR="/opt/mihomo-party"
DATA_DIR="/var/lib/mihomo-party"
MIHOMO_CONFIG_DIR="/etc/mihomo"
MIHOMO_BINARY="/usr/local/bin/mihomo"
WEB_PORT="${WEB_PORT:-8080}"
SERVER_PORT="${SERVER_PORT:-8090}"
REPO_URL="https://github.com/raylenzed/mihomo-party"
...
log_banner()  {
  ...
  echo -e "${PURPLE}${BOLD}║          Mihomo Party — Install Script               ║${NC}"
  echo -e "${PURPLE}${BOLD}╚══════════════════════════════════════════════════════╝${NC}"
  echo -e ""
}
...
# Mihomo configuration — generated by mihomo-party installer
# Edit via the Mihomo Party web UI or directly here.
...
secret: ""
external-ui: /opt/mihomo-party/ui
...
# ─── 7. install_mihomo_party ──────────────────────────────────────────────────

install_mihomo_party() {
  log_step "Installing Mihomo Party web dashboard"

  mkdir -p "$DATA_DIR"

  # Check if already installed
  if [[ -d "$INSTALL_DIR" ]]; then
    log_warn "Mihomo Party already installed at $INSTALL_DIR"
    if ! confirm "Re-install / upgrade?"; then
      log_info "Skipping Mihomo Party install"
      return 0
    fi
  ...
  log_info "Mihomo Party built successfully"
}
...
EOF
  ...
  # ── mihomo-party.service ──────────────────────────────────────────────
  local node_bin
  node_bin="$(command -v node)"
  cat > /etc/systemd/system/mihomo-party.service <<EOF
[Unit]
Description=Mihomo Party - Web Dashboard
...
SyslogIdentifier=mihomo-party

[Install]
WantedBy=multi-user.target
EOF
  log_info "Created /etc/systemd/system/mihomo-party.service"

  # Reload and enable
  systemctl daemon-reload
  systemctl enable mihomo.service
  systemctl enable mihomo-party.service
  ...
  log_detail "Starting mihomo-party..."
  systemctl start mihomo-party.service
  sleep 2

  if systemctl is-active --quiet mihomo-party.service; then
    log_info "Mihomo Party is running"
  else
    log_warn "Mihomo Party failed to start — check: journalctl -u mihomo-party -n 50"
  fi
}
...
configure_firewall() {
  ...
  if command -v ufw &>/dev/null && ufw status | grep -q "active"; then
    ...
    if confirm "Open ports ${WEB_PORT} (UI), ${SERVER_PORT} (API), 7890 (proxy) in ufw?"; then
      ufw allow "$WEB_PORT/tcp"   comment "mihomo-party UI"   &>/dev/null
      ufw allow "$SERVER_PORT/tcp" comment "mihomo-party API"  &>/dev/null
...
show_summary() {
  ...
  echo -e "${GREEN}${BOLD}╚══════════════════════════════════════════════════════╝${NC}"
  echo -e ""
  echo -e "${BOLD}  Mihomo Party Web UI:${NC}"
  ...
  echo -e "  ${CYAN}systemctl status mihomo-party${NC}     — dashboard status"
  echo -e "  ${CYAN}journalctl -fu mihomo${NC}             — core logs (live)"
  echo -e "  ${CYAN}journalctl -fu mihomo-party${NC}       — dashboard logs (live)"
...
uninstall() {
  log_banner
  log_step "Uninstalling Mihomo Party"

  echo -e "${RED}${BOLD}  WARNING: This will remove:${NC}"
  echo -e "  • Mihomo binary ($MIHOMO_BINARY)"
  echo -e "  • Mihomo Party ($INSTALL_DIR)"
  echo -e "  • Mihomo Party data ($DATA_DIR)"
  ...
  for svc in mihomo-party mihomo; do
